...
impl Server {
    ...
    async fn listen_downstream_http(&self) -> Result<()> {
        ...
        let listener = match TcpListener::bind(&https_bind_addr).await {
        ...
        let result = async {
            ...
            loop {
                ...
                if !is_chunked || content_len.is_some_and(|len| len < STREAM_THRESHOLD) {
                    ...
                    debug!("Forwarding request header to upstream via {stream_id}");
                    let message = Message::Data(Data::Http(request));
                    protocol::write_message_locked(&mut upstream, &message).await?;
                ...
                match response {
                    Message::StreamOpen {
                        ...
                        let response_header = protocol::read_message_locked(&mut upstream).await?;
                        debug!(
